...
export function DataTable<T>({ data, columns, dense = false }: DataTableProps<T>) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-white/[0.10] bg-white/[0.035] shadow-[0_18px_60px_rgba(0,0,0,0.22)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-3 border-b border-white/[0.08] bg-black/20 p-3">
        <input
          value={globalFilter}
          onChange={(event) => setGlobalFilter(event.target.value)}
          placeholder="搜索期号、公式、结果..."
          className="h-9 w-full max-w-sm rounded-lg border border-white/10 bg-white/[0.055] px-3 text-sm text-slate-100 outline-none placeholder:text-slate-600 focus:border-cyan-300/40 focus:ring-2 focus:ring-cyan-300/10"
        />
      ...
      <div className="flex items-center justify-between gap-3 border-t border-white/[0.08] bg-black/20 px-3 py-2 text-xs text-slate-400">
